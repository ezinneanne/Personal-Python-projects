...
def callback(r,c):
    ...
    if player == "X" and states[x][y] == 0 and stop_game==False:
        b[r][c].configure(text="X",fg='white',bg='blue')
        states[r][c] = "X"
        player = "O"
        
        
    if player == "O" and states[r][c] == 0 and stop_game==False:
        b[r][c].configure(text='O',fg='black',bg='orange')
        states[r][c] = 'O'
        player = 'X'
...
for i in range(3):
        for j in range(3):
                b[i][j] = Button(font=('Verdana',16), width=3, bg='yellow',command=lambda r=i, c=j: callback(r,c))
